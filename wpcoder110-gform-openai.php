...
function wpcoder110_get_feeds( $form_id = null ) {
    ...
    $sql = $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}gf_addon_feed
                               WHERE addon_slug=%s {$form_filter} ORDER BY `feed_order`, `id` ASC", 'gravityforms-openai'
    );
...
function wpcoder110_ajax_calls(){
    ...
    <script>
        ...
        var div_index = 0;
    const source = new EventSource("<?php echo admin_url('admin-ajax.php'); ?>?action=event_stream_openai&form_id=<?php echo $form_id; ?>&entry_id=<?php echo $entry_id; ?>&nonce=<?php echo wp_create_nonce('ssevent_stream_openai'); ?>");
...
function wpcoder110_make_request($feed, $entry, $form) {
    ...
    if ($endpoint === 'chat/completions') {
        ...
        $GWiz_GF_OpenAI_Object->log_debug( __METHOD__ . '(): ' . sprintf( __( 'Sent request to OpenAI. Feed: %1$s, Endpoint: chat, Model: %2$s, Message: %3$s', 'gravityforms-openai' ), $feed['meta']['feed_name'], $model, $message ) );
        ...
        $body['max_tokens'] = (float) rgar( $feed['meta'], $endpoint . '_' . 'max_tokens', $GWiz_GF_OpenAI_Object->default_settings['chat/completions']['max_tokens'] );
        $body['temperature'] = (float) rgar( $feed['meta'], $endpoint . '_' . 'temperature', $GWiz_GF_OpenAI_Object->default_settings['chat/completions']['temperature'] );
        $body['top_p'] = (float) rgar( $feed['meta'], $endpoint . '_' . 'top_p', $GWiz_GF_OpenAI_Object->default_settings['chat/completions']['top_p'] );
        $body['frequency_penalty'] = (float) rgar( $feed['meta'], $endpoint . '_' . 'frequency_penalty', $GWiz_GF_OpenAI_Object->default_settings['chat/completions']['frequency_penalty'] );
        $body['presence_penalty']  = (float) rgar( $feed['meta'], $endpoint . '_' . 'presence_penalty', $GWiz_GF_OpenAI_Object->default_settings['chat/completions']['presence_penalty'] );
        ...
        curl_setopt($ch, CURLOPT_WRITEFUNCTION, function ($ch, $data) use ($object) {
            ...
            if (isset($pop_arr[1])) {
                ...
                if (isset($pop_js['choices'])) {
                    $line = isset($pop_js['choices'][0]['delta']['content']) ? isset($pop_js['choices'][0]['delta']['content']) : '';
                    if ($line) {
                        if(strpos($line, "\n") !== FALSE) {
                            $object->res .= nl2br($pop_js['choices'][0]['delta']['content']);
                        } else {
                            $object->res .= $pop_js['choices'][0]['delta']['content'];
                        }
                    }
                }
            } else {
            ...
            wpcoder110_chatgpt_writelog(print_r($data, true)); 
            echo $data;
            return strlen($data);
        });
        ...
        if (!empty($object->res)) {
            GFAPI::add_note( $entry['id'], 0, 'OpenAI Response (' . $feed['meta']['feed_name'] . ')', $object->res );
            $entry = $GWiz_GF_OpenAI_Object->maybe_save_result_to_field( $feed, $entry, $form, $object->res );
        } else {
            $GWiz_GF_OpenAI_Object->add_feed_error( $object->error, $feed, $entry, $form );
            return $object;
        }        

        gform_add_meta( $entry['id'], 'openai_response_' . $feed['id'], $object->res );
...
function event_stream_openai(){
    if ( ! headers_sent() ) {
        ...
        foreach ( $feeds as $feed ) {
            ...
            $end_point = isset($feed['meta']['endpoint']) ? $feed['meta']['endpoint'] : '';

            $field_id = isset($feed['meta'][$end_point . '_map_result_to_field']) ? $feed['meta'][$end_point . '_map_result_to_field'] : 0;
            ...
            if ( in_array((string)$feed['id'], $processed_feeds) ) {
                ...
                foreach ($lines as $line) {
                    ...
                    $object->content = $line;
                    echo 'data: ' . json_encode(array("choices" => array(array("delta" => $object)))) . PHP_EOL;
                    echo PHP_EOL;
                    flush();
                }
...
function wpcoder110_chatgpt_writelog($log_text_line){
	if(OPAIGFRLT_LOG) {
		...
		file_put_contents(OPAIGFRLT_PATH . 'logs/' . $filename, $log_text_line.PHP_EOL , FILE_APPEND | LOCK_EX);
	}
}
